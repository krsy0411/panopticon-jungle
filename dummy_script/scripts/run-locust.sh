...
# This script helps run Locust tests with various configurations
...
echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}🦗 Locust Load Test Runner${NC}"
echo -e "${BLUE}=================================================${NC}"
echo ""
...
echo "  1) Standard Test - Exactly 1M logs (10 users, Headless)"
...
case $choice in
    1)
        echo -e "${GREEN}Starting standard load test...${NC}"
        echo -e "${YELLOW}Goal: Exactly 1,000,000 logs (no time limit)${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-1m-$(date +%Y%m%d-%H%M%S).html"
        ;;
    2)
        ...
        echo -e "${YELLOW}  - Test will auto-stop at 1M logs${NC}"
        echo ""
        $LOCUST_CMD -f "$LOCUST_FILE" --host "$PRODUCER_URL" --html "${REPORTS_DIR}/locust-webui-$(date +%Y%m%d-%H%M%S).html"
        ;;
    3)
        echo ""
        ...
        if [[ "$headless" == "y" || "$headless" == "Y" ]]; then
            ...
            echo -e "${YELLOW}Will run until 1M logs are sent${NC}"
            mkdir -p "$REPORTS_DIR"
            $LOCUST_CMD -f "$LOCUST_FILE" \
                   --host "$PRODUCER_URL" \
                   --users "$users" \
                   ...
                   --headless \
                   --print-stats \
                   --html "${REPORTS_DIR}/locust-custom-$(date +%Y%m%d-%H%M%S).html"
        else
            echo -e "${GREEN}Starting custom load test in Web UI mode...${NC}"
            echo -e "${YELLOW}Open http://localhost:8089 in your browser${NC}"
...
if [ -d "$REPORTS_DIR" ]; then
    LATEST_REPORT=$(ls -t "${REPORTS_DIR}"/locust-*.html 2>/dev/null | head -1)
