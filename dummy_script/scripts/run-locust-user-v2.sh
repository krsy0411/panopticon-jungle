...
# Locust User Simulation Test Runner V2 (Service-Aware Endpoints)
# This script helps run user simulation tests with realistic, service-matched logs and spans
...
LOCUST_FILE="${SCRIPT_DIR}/locustfile_user_v2.py"
REPORTS_DIR="$( cd "${SCRIPT_DIR}/.." && pwd )/reports"

echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}🦗 Locust User Simulation Test Runner V2${NC}"
echo -e "${BLUE}   (Service-Aware Endpoints)${NC}"
echo -e "${BLUE}=================================================${NC}"
echo ""
...
case $choice in
    1)
        echo -e "${GREEN}Starting 10-minute user simulation test (V2)...${NC}"
        echo -e "${YELLOW}Duration: 10 minutes${NC}"
        echo -e "${YELLOW}Users: 5 concurrent users${NC}"
        echo -e "${YELLOW}Data: Service-aware endpoints (Logs 60% + Spans 40%), 2% error rate${NC}"
        echo ""
        mkdir -p "$REPORTS_DIR"
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-v2-10m-$(date +%Y%m%d-%H%M%S).html"
        ;;
    2)
        echo -e "${GREEN}Starting 1-hour user simulation test (V2)...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-v2-1h-$(date +%Y%m%d-%H%M%S).html"
        ;;
    3)
        echo -e "${GREEN}Starting 3-hour user simulation test (V2)...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-v2-3h-$(date +%Y%m%d-%H%M%S).html"
        ;;
    4)
        echo -e "${GREEN}Starting 24-hour user simulation test (V2)...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-v2-24h-$(date +%Y%m%d-%H%M%S).html"
        ;;
    5)
        echo -e "${GREEN}Starting Locust in Web UI mode (V2)...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               --host "$PRODUCER_URL" \
               --html "${REPORTS_DIR}/locust-user-v2-webui-$(date +%Y%m%d-%H%M%S).html"
        ;;
    6)
        echo ""
        ...
        if [[ "$headless" == "y" || "$headless" == "Y" ]]; then
            echo -e "${GREEN}Starting custom user simulation test (V2)...${NC}"
            ...
            $LOCUST_CMD -f "$LOCUST_FILE" \
                   ...
                   --headless \
                   --print-stats \
                   --html "${REPORTS_DIR}/locust-user-v2-custom-$(date +%Y%m%d-%H%M%S).html"
        else
            echo -e "${GREEN}Starting custom test in Web UI mode (V2)...${NC}"
            echo -e "${YELLOW}Open http://localhost:8089 in your browser${NC}"
...
if [ -d "$REPORTS_DIR" ]; then
    LATEST_REPORT=$(ls -t "${REPORTS_DIR}"/locust-user-v2-*.html 2>/dev/null | head -1)
