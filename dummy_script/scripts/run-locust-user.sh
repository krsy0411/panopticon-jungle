...
# Locust User Simulation Test Runner
# This script helps run user simulation tests with realistic logs and spans
...
LOCUST_FILE="${SCRIPT_DIR}/locustfile_user.py"
REPORTS_DIR="$( cd "${SCRIPT_DIR}/.." && pwd )/reports"

echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}🦗 Locust User Simulation Test Runner${NC}"
echo -e "${BLUE}=================================================${NC}"
echo ""
...
case $choice in
    1)
        echo -e "${GREEN}Starting 10-minute user simulation test...${NC}"
        echo -e "${YELLOW}Duration: 10 minutes${NC}"
        echo -e "${YELLOW}Users: 5 concurrent users${NC}"
        echo -e "${YELLOW}Data: Logs (80%) + Spans (20%), 1% error rate${NC}"
        echo ""
        mkdir -p "$REPORTS_DIR"
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-10m-$(date +%Y%m%d-%H%M%S).html"
        ;;
    2)
        echo -e "${GREEN}Starting 1-hour user simulation test...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-1h-$(date +%Y%m%d-%H%M%S).html"
        ;;
    3)
        echo -e "${GREEN}Starting 3-hour user simulation test...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-3h-$(date +%Y%m%d-%H%M%S).html"
        ;;
    4)
        echo -e "${GREEN}Starting 24-hour user simulation test...${NC}"
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               ...
               --headless \
               --print-stats \
               --html "${REPORTS_DIR}/locust-user-24h-$(date +%Y%m%d-%H%M%S).html"
        ;;
    5)
        ...
        $LOCUST_CMD -f "$LOCUST_FILE" \
               --host "$PRODUCER_URL" \
               --html "${REPORTS_DIR}/locust-user-webui-$(date +%Y%m%d-%H%M%S).html"
        ;;
    6)
        echo ""
        ...
        if [[ "$headless" == "y" || "$headless" == "Y" ]]; then
            echo -e "${GREEN}Starting custom user simulation test...${NC}"
            ...
            $LOCUST_CMD -f "$LOCUST_FILE" \
                   ...
                   --headless \
                   --print-stats \
                   --html "${REPORTS_DIR}/locust-user-custom-$(date +%Y%m%d-%H%M%S).html"
        else
            echo -e "${GREEN}Starting custom test in Web UI mode...${NC}"
            echo -e "${YELLOW}Open http://localhost:8089 in your browser${NC}"
...
if [ -d "$REPORTS_DIR" ]; then
    LATEST_REPORT=$(ls -t "${REPORTS_DIR}"/locust-user-*.html 2>/dev/null | head -1)
