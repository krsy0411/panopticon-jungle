...
const API_BASE_URL = "https://api.jungle-panopticon.cloud/producer";
// 'http://localhost:3005/producer';

// 설정
const CONFIG = {
  START_TIME: new Date("2025-11-12T00:00:00+09:00"),
  END_TIME: new Date("2025-11-12T15:00:00+09:00"),
  TOTAL_SPANS: 0, // 생성할 span 개수
  TOTAL_LOGS: 100000, // 생성할 log 개수
...
function generateLog(): any {
  ...
  const payload = {
    ...
    service_name: "panopService",
    environment: "prod",
    level: "INFO",
    message: "샘플 APM 로그 이벤트",
    trace_id: "sadasdsadkdasd",
    span_id: "asdasadaa",
    http_method: "GET",
...
async function main() {
  ...
  try {
    ...
    console.log(
      `\nSummary: ${CONFIG.TOTAL_LOGS} logs + ${CONFIG.TOTAL_SPANS} spans = ${
        CONFIG.TOTAL_LOGS + CONFIG.TOTAL_SPANS
      } total records`
    );
